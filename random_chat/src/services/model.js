...
export default class Model extends CreateEvent {
    ...
    onSuccess() {
        ...
        return new Promise(
            (resolve) => (this.indexedDBMessages.onsuccess = (e) => resolve(e.target.result))
        );
    }
    onError() {
        return new Promise(
            (resolve) => (this.indexedDBMessages.onerror = (e) => resolve(e.target.result))
        );
    }
    onCreate(payload) {
        this.onSuccess().then(
            (database) =>
                (database
                    ...
                    .add(payload).onsuccess = (e) => this.trigger('messagechange', payload))
        );
    }
    getAll() {
        return new Promise((resolve) =>
            this.onSuccess().then(
                (database) =>
                    (database.transaction('messages').objectStore('messages').getAll().onsuccess = (
                        e
