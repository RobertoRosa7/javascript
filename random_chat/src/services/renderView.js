...
export default class RenderView extends CreateEvent {
    ...
    renderingPdf(file, self) {
        return `
					<span id="show-total" class="slide-length">1/${
                        self.el['containerDocumentPreview'].childElementCount + 1
                    }</span>
					<span id="close-slide" class="close-slide"><i data-images="${
                        file.id
                    }" class="small material-icons">close</i></span>
					<img src="${file.src}">
					<div class="caption">Arquivo em formato de ${Format.formatNameFromImage(
                        file.info.type
                    )} - páginas ${file.pages}</div>                
			`;
    }
