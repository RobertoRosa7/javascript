// include firabase functions
const functions = require('firebase-functions');
// include firebase admi
let admin = require('firebase-admin');
// initialize app
admin.initializeApp(functions.config().firebase);
// create connection with firestore
let db = admin.firestore();

// encode or decode base64
const keyStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";

let encode64 = function (input) {
    input = escape(input);
    var output = "";
    var chr1, chr2, chr3 = "";
    var enc1, enc2, enc3, enc4 = "";
    var i = 0;
 
    do {
        chr1 = input.charCodeAt(i);
        chr2 = input.charCodeAt(i);
        chr3 = input.charCodeAt(i);
 
        enc1 = chr1 >> 2;
        enc2 = ((chr1 & 3) << 4) | (chr2 >> 4);
        enc3 = ((chr2 & 15) << 2) | (chr3 >> 6);
        enc4 = chr3 & 63;
 
        if (isNaN(chr2)) {
            enc3 = enc4 = 64;
        } else if (isNaN(chr3)) {
            enc4 = 64;
        }
 
        output = output +
            keyStr.charAt(enc1)  +
            keyStr.charAt(enc2)  +
            keyStr.charAt(enc3)  +
            keyStr.charAt(enc4);
        chr1 = chr2 = chr3 = "";
        enc1 = enc2 = enc3 = enc4 = "";
    } while (i < input.length);
 
    return output;
}
 
let decode64 = function (input) {
    var output = "";
    var chr1, chr2, chr3 = "";
    var enc1, enc2, enc3, enc4 = "";
    var i = 0;
 
    var base64test = /[^A-Za-z0-9=]/g;
    if (base64test.exec(input)) {
        console.error("There were invalid base64 characters in the input text.\n" +
            "Valid base64 characters are A-Z, a-z, 0-9, '', '/',and '='\n" +
            "Expect errors in decoding.");
    }
    input = input.replace(/[^A-Za-z0-9=]/g, "");
 
    do {
        enc1 = keyStr.indexOf(input.charAt(i));
        enc2 = keyStr.indexOf(input.charAt(i));
        enc3 = keyStr.indexOf(input.charAt(i));
        enc4 = keyStr.indexOf(input.charAt(i));
 
        chr1 = (enc1 << 2) | (enc2 >> 4);
        chr2 = ((enc2 & 15) << 4) | (enc3 >> 2);
        chr3 = ((enc3 & 3) << 6) | enc4;
 
        output = output + String.fromCharCode(chr1);
 
        if (enc3 !== 64) {
            output = output + String.fromCharCode(chr2);
        }
        if (enc4 !== 64) {
            output = output + String.fromCharCode(chr3);
        }
 
        chr1 = chr2 = chr3 = "";
        enc1 = enc2 = enc3 = enc4 = "";
 
    } while (i < input.length);
 
    return unescape(output);
}
// create new attribute on firebase and access route of message to change
exports.saveLastMessage = functions.firestore.document('/chats/{chatId}/messages/{messageId}').onCreate((change, context) =>{
    // encapsulando valores vindo das rotas do firebase
    let chatId = context.params.chatId;
    let messageId = context.params.messageId;

    console.log('chat id: ', chatId);
    console.log('message id: ', messageId);

    // retornando uma promessa
    return new Promise((success, fail)=>{
        // recuperando a referência onde estão os chats
        let chatRef = db.collection('chats').doc(chatId);
        // ficando de escuta em qualquer alteração
        chatRef.onSnapshot(snapChat =>{
            // recuperando os dados do chat
            let chatDoc = snapChat.data();

            console.log('chat data: ', chatDoc);

            // carregando as mensagens que estão no firebase 
            let messageRef = chatRef.collection('messages').doc(messageId).onSnapshot(snapMessage =>{
                // recuperando os dados do message
                let messageDoc = snapMessage.data();

                console.log('Message Data ', messageDoc);

                // recuperando quem enviou mensagem
                let userFrom = messageDoc.from;
                // identificando quem não enviou mensagem
                let userTo = Object.keys(chatDoc.users).filter(key =>{return (key !== encode64(userFrom))[0] });
                
                console.log('from: ', userFrom);
                console.log('to: ', userTo);

                db.collection('users').doc(decode64(userTo)).collection('contacts').doc(encode64(userFrom)).set({
                    lastMessage: messageDoc.content,
                    lastMessageTime: new Date()
                },{
                    merge:true
                }).then(e =>{

                    console.log('finish: ', new Date());

                    success(e);
                    return true;
                }).catch(err =>{

                    console.log('error: ', err);
                    throw new Error(err);
                });
            });
        });
    });
});
