...
export class Messages extends Model{
    ...
    // send the message
     static send(chatId, from, type, content){
        // get contact referense on firebase
        return new Promise((success, fail) =>{
            Messages.getRef(chatId).add({
                content,
                timeStamp: new Date(),
                from,
                status:'wait',
   ...
   // send image
    static sendImage(chatId, from, file){
        return new Promise((success, fail)=>{
            Messages.upload(file).then(snapshot =>{
                Messages.send(chatId,from,'image', snapshot.downloadURL).then(()=>{
                    success();
                });
            });
        });
    ...
    static sendDocument(chatId, from, file, preview, info){
        // mensagem antes de upload para avisar que foi enviada
        Messages.send(chatId, from, 'document','').then(msgRef =>{
            // upload e get download URL from document
            Messages.upload(file, from).then(snapshot =>{
                let downloadFile = snapshot.downloadURL;
                // se não for pdf
                if(preview){
                    // upload e get preview
                    Messages.upload(preview, from).then(snapshot2 =>{
    ...
    // upload of files, documents or image to firestorage
    static upload(file, from){
       return Upload.send(file, from);
    }
    // get view of content element
    getViewElement(me = true){
        ...
        switch(this.type){
            ...
            case 'document':
                div.innerHTML = `
                    <div class="_3_7SH _1ZPgd view-document">
                ...
                `;
                // download document
                div.querySelector('.view-document').on('click', e =>{
            ...
            case 'audio':
                ...
                let audioPlay = div.querySelector('.audio-play');
                let audioPause = div.querySelector('.audio-pause');
                ...
                audioEl.ontimeupdate = e =>{
                    ...
                    (audioEl.paused) ? audioPlay.show() : audioPause.show();
                };
                audioPlay.on('click', e =>{
                    audioEl.play();
                });
                audioPause.on('click', e =>{
                    audioEl.pause();
                });
                ...
                break;
            default:
            // id foi colocado aqui para identificar as mensagens
            // porém, os outro tipo de mensagem estavam sendo duplicados
            // deve colocar id na div e não aqui.
                div.innerHTML = `
        ...
        // message send or receive to applay class
        let className = 'message-in';
        if(me){
            className = 'message-out';
            // apply style message out
            div.firstElementChild.classList.add(className);
            // apply messages status
            div.querySelector('.message-time').parentElement.appendChild(this.getStatusViewElement());
        }
        // apply on first element of div
        div.firstElementChild.classList.add(className);
        return div;
    }
    // get referenses chat on collection messages
