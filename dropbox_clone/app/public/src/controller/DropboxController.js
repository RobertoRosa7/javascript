...
class DropboxController{
	...
	connectFirebase(){
		 // Initialize Firebase
	    var config = {
	    apiKey: "AIzaSyDtIWtRbYFgNjj83FIQxSwUM-FJmogVYtk",
	    authDomain: "dropboxclone-29e56.firebaseapp.com",
	    databaseURL: "https://dropboxclone-29e56.firebaseio.com",
	    projectId: "dropboxclone-29e56",
	    storageBucket: "dropboxclone-29e56.appspot.com",
	    messagingSenderId: "1070559009438"
	    };
	    firebase.initializeApp(config);
	}
	// get firebase referenses
	firebaseReferense(path){
		if(!path) path = this.currentFolder.join('/');
		return firebase.database().ref(path);
	}
	// get itens selected on list of elments
	getSelection(){
		return this.listFiles.querySelectorAll('.selected');
	}
	removeFolder(ref, name){
		return new Promise((resolve, reject)=>{
			let folderRef = this.firebaseReferense(ref + '/' + name);
	...
	// remove arquivos do storage e database
	removeFile(ref, name){
		let fileRef = firebase.storage().ref(ref).child(name);
		return fileRef.delete();			
	}
	// remove task, one or more files
	removeTask(){
		let promises = [];
		this.getSelection().forEach(li =>{
			// removendo tarefa do storage firebase
			...
			// removendo tarefa do servidor express
			/*
			let file = JSON.parse(li.dataset.file);
			let key = li.dataset.key;
	...
	initEvents(){
		// evento to add new folder
		this.btnNewFolder.addEventListener('click', e =>{
			let name = prompt('Nova pasta:');
			if(name){
				this.firebaseReferense().push().set({
					name,
					type: 'folder',
		...
		// event to delete file on disk and on firebase
		this.btnDelete.addEventListener('click', e => {
			this.removeTask().then(responses => {
				responses.forEach(resp =>{
					if(resp.fields.key){
						this.firebaseReferense().child(resp.fields.key).remove();
					}
				});
			}).catch(err =>{
				console.error(err);
		...
		this.btnRename.addEventListener('click', e =>{
			...
			if(name){
				file.name = name;
				this.firebaseReferense().child(li.dataset.key).set(file);
			}
		});
		// event to listen any change on selection
		...
		this.inputFiles.addEventListener('change', e => {
			...
			this.uploadTask(e.target.files).then(responses =>{
				...
				this.firebaseReferense().push().set({
					...
					type:data[1].type,
					size:data[1].size,
					content:data[1].contentType,
					data:data[1].timeCreated,
					fullPath:data[1].fullPath,
					path:data[0]
				});

				// adicionar arquivos vindo do firebase storage para firebase databse
				/*
				responses.forEach(resp =>{
					this.firebaseReferense().push().set({
						name:resp.name,
				...
				// adição de arquivos no firebase, servidor express
				/*
				responses.forEach(resp =>{
					this.firebaseReferense().push().set(resp.files['input-file']);
				});
				*/
				this.uploadComplete();
			}).catch(err => {
	...
	// upload complete remove modal and clear
	uploadComplete(){
		this.showBarProgress(false);
		this.inputFiles.value = '';
		this.inputFiles.disabled = false;
	}
	// ajax to add and to remove files on disk
	...
	// make upload of files - return Promise
	uploadTask(files){
		let promises = [];
		let dataURL = [];
		let data = [];
		...
		[...files].forEach(file =>{
			promises.push(new Promise((resolve, reject) =>{
				// método de gravação no storage firebase
				...
				task.on('state_changed',snapshot =>{
					...
					fileRef.getMetadata().then(metadata =>{
						data.push(metadata)
						dataComplete.push(dataURL);
						dataComplete.push(data);
						resolve(dataComplete);
					}).catch(err =>{
						console.error(err);
					})
				});
			...
			// método de gravação no servidor express
			/*
			let formData = new FormData();
	...
	getFileView(file, key){
		...
		li.innerHTML = `
            ...
            <div class="name text-center text-ellipsi">${file.name}</div>
		`;
		this.eventsLI(li);
		return li;
	}
	// read files from firebase
	readFiles(){
		this.lastFolder = this.currentFolder.join('/');
		this.firebaseReferense().on('value', snapshot => {
	...
	// open folder and read file into folder
	openFolder(){
		if(this.lastFolder)this.firebaseReferense(this.lastFolder).off('value');
		this.renderNav();
		this.readFiles();
	}
	// render the menu navegation when the folder is opened
	renderNav(){
	...
	// select the files to edit or remove
	eventsLI(li){
